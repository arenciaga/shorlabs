  ...
  return (
    <Card className="flex flex-col h-full rounded-none border-0 bg-transparent py-0 shadow-none gap-0">
      ...
      <div className="flex-1 flex flex-col justify-center gap-5 sm:gap-6 p-4 sm:p-6">
        <ScrambleNumber target="14+" label="Frameworks" />
        <ScrambleNumber target="Auto" label="Detection" />
        <ScrambleNumber target="Real-time" label="Deploy Logs" />
        <ScrambleNumber target="Auto SSL" label="Custom Domains" />
      </div>
    </Card>
  )
}
